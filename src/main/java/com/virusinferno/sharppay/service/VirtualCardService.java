...
public class VirtualCardService {
    ...
    public CardResponse getMyCard(String email) {
        User user = userRepository.findByEmail(email).orElseThrow(() -> new RuntimeException("User not found!"));
        VirtualCard card = virtualCardRepository.findByUser(user).orElseThrow(() -> new RuntimeException("No active virtual card found!"));
        return mapToResponse(card, user.getFullName());
    }
    ...
    public String updateCardStatus(String email, String newStatus) {
        User user = userRepository.findByEmail(email).orElseThrow(() -> new RuntimeException("User not found!"));
        VirtualCard card = virtualCardRepository.findByUser(user).orElseThrow(() -> new RuntimeException("No virtual card found for this account."));
        ...
        virtualCardRepository.save(card);
        return "Card status updated to " + newStatus;
    }

    private String generateCardNumber(String type) {
        Random rand = new Random();
        StringBuilder pan = new StringBuilder();
        pan.append(type != null && type.equals("VISA") ? "4" : "5");
        for (int i = 0; i < 15; i++) pan.append(rand.nextInt(10));
        return pan.toString();
    }

    private String generateExpiryDate() {
        return LocalDate.now().plusYears(3).format(DateTimeFormatter.ofPattern("MM/yy"));
    }
